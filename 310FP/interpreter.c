...
int interpreter(char *parsedCommand[]){
    ...
    // New commands for final project TODO
        ...
        return readpipeCommandEXAM(parsedCommand);
...
int printCommand(char *parsedCommand[]){
    ...
    if (isalnum(*value)){
        ...
        printf("print Variable does not exist\n");
    }
    return 0;
}
...
void printOwners(){
  printf("\n owners: ");
  for (int i = 0; i < MAX_FILES; i++){
    printf("%d ", active_file_table_owners[i]);
  }
}
...
////////////////////////////////////////////////////////////////
    if (parsedCommand[1] == NULL && parsedCommand[2]==NULL){
      printf("Wrong Syntax. The write_EXAM command requires a file ID and a string enclosed by \"[]\" as arguments. \n");
      return 1;
    }
    int actindex = atoi(parsedCommand[1]);

    if (active_file_table_owners[actindex] == -1){// this be dying
       printf("ERROR: Open the file first.\n");
       return 2;
    }
//////////////////////////////////////////////////////////////////
    //printf("Init pointer.\n");
    int strLength = strlen(parsedCommand[2]);
    float blockSize = getBlockSize();
    int blocksToWrite = (strLength / blockSize) + 1;

    for (int i = 0; i < blocksToWrite; i++){
      int ptrMovement = i * getBlockSize();
      char *ptr = (parsedCommand[2] + ptrMovement);
      writeEXAMFile(fatindex, ptr);
    }
    //closeFile(actindex);
    return 0;
}
...
int readCommandEXAM(char *parsedCommand[]){
    ...
    if (parsedCommand[1] == NULL && parsedCommand[2] == NULL){
      printf("Wrong syntax. The open_EXAM command requires a file ID and a filename.\n");
      return 1;
    }
    int actindex = atoi(parsedCommand[1]);
...
//    printf("Running Read Command \n");
    printf("CURR LOC READ_EXAM: %d \n", fat[actindex].current_location);
    char *ptr = readEXAMBlock(actindex);
    if (ptr == NULL){
    ...
    fat[actindex].current_location = fat[actindex].file_length;
    printf("AFTER READ1 CURR LOC: %d \n", fat[actindex].current_location);

    printf("VARIABLE SET IN: %s \n", parsedCommand[2]);
...
int openCommandEXAM(char *parsedCommand[]){
    ...
    if(parsedCommand[1] == NULL && parsedCommand[2] == NULL){
      printf("Wrong syntax. Open_EXAM takes a file ID and a filename.\n");
      return 1;
    }

    int actindex = atoi(parsedCommand[1]);
    if (active_file_table_owners[actindex] != -1){
      //printf("ACT INDEX ERROR: %d \n", active_file_table_owners[actindex]);
      printf("ERROR: Index number in use. \n");
      return 2;}
    fatindex = openEXAMFile(actindex, parsedCommand[2]);
    if (fatindex < 0)
      printf("ERROR occured. Error codes here");
...
int closeCommandEXAM(char *parsedCommand[]){
    ...
    if(parsedCommand[1] == NULL){
      printf("Wrong syntax. close_EXAM takes a file ID.\n");
      return 1;
    }
  //  printOwners();
  //  printActive();
  ...
  //  printf("Entering closeExamFile");
    closeEXAMFile(actindex);
    free(pipeBuffer);
...
int seekCommandEXAM(char *parsedCommand[]){
    ...
    printf("IN SEEK \n");
    if(parsedCommand[1] == NULL || parsedCommand[2] == NULL){
      printf("Wrong syntax. seek_EXAM takes a file ID and an offset.\n");
      return 1;
    }
    ...
    printf("offset :) %d \n", offset);
     //account for negative offset inputs:

    printf("CURR LOC: %d \n", fat[actindex].current_location);
    if (offset + fat[actindex].current_location <= 0){
      FILE *charptr = active_file_table[actindex];
      int blkToRead = fat[actindex].blockPtrs[fat[actindex].current_location];
      printf("Block to read: %d \n", blkToRead);


      fseekToBlock(charptr, blkToRead);

      int i = 0;

      for(; i < currentPartition.block_size * fat[actindex].current_location; i++){
        char c = fgetc(charptr);
        printf("%c", c);
      //  count++;
        if(c == '0'){
          //  printf("%c", fgetc(charptr));
            break;
        }
        //if (fgetc(charptr) == ' ') printf("HII");
    //    printf("%c", fgetc(charptr));
        count = count + 1;
      }
      printf("%c", fgetc(charptr));
    //  count = (currentPartition.last_free_block*currentPartition.block_size) - count;
      printf("\nCHAR COUNT: %d \n", count);
      offset = count + offset;
      printf("OFFSET -ve: %d \n", offset);
      OFFSETS[actindex] = OFFSETS[actindex] + offset;
      if(OFFSETS[actindex] == count){
        OFFSETS[actindex] == 0;
      }
    } else {
      OFFSETS[actindex] = OFFSETS[actindex] + offset;
    ...
    int output = seekEXAMFile(actindex, offset);
    printf("########## fatindex: %d \n", output);
...
int readpipeCommandEXAM(char *parsedCommand[]){
    if(parsedCommand[1] == NULL || parsedCommand[2] == NULL){
      printf("Wrong syntax. readpipe_EXAM takes a file ID and a var.\n");
      return 1;
    }
    printf("IM HEREE \n");
    // normal read_EXAM case
    if (parsedCommand[3] == NULL){
      printf("Standard Read EXAM \n");
      readCommandEXAM(parsedCommand);
      //return 0;
    }
    else {
      int actindex = atoi(parsedCommand[1]);
      ...
      if (actindex == -1){
        printf("Failed to open file EXAM. \n");
        return 2;
      }

      char *ptr = readpipeCommand(actindex);

      if(ptr == NULL ){
        printf("Read pipe command failed. \n");
        return 2;
      }
      printf("Hi fam i here.\n");
      char *pipeStr = malloc(MAX_BUFFER);
      strcpy(pipeStr, ptr);
      while(ptr != NULL){
        ptr = readpipeCommand(actindex);
        if (ptr == NULL){
          break;
        }
        strcat(pipeStr, ptr);
      ...
      setVariable(parsedCommand[4], pipeStr);
      fat[actindex].current_location = fat[actindex].file_length;
      printf("AFTER READ2 CURR LOC: %d \n", fat[actindex].current_location);

      return 0;
    //char* var = parsedCommand[4];
